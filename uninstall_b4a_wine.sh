#!/bin/bash
#===============================================================================
# B4A Wine Uninstaller for Linux Mint
# Safely removes B4A, Wine prefix, launchers, and optional folders
# Author: pyhoon
...
readonly DESKTOP_SHORTCUT="${HOME}/Desktop/b4a-wine.desktop"
readonly LOCAL_ICON="${HOME}/.local/share/icons/b4a.png"
readonly PROJECTS_DIR="${HOME}/B4A_Projects"
...
Safely uninstall B4A Wine installation from Linux Mint.

OPTIONS:
    -h, --help          Show this help message and exit
    -d, --dry-run       Show what would be removed without deleting anything
    -f, --force         Skip confirmation prompts (use with caution!)
    -p, --keep-projects Keep the ~/B4A_Projects folder (default: remove)
...
check_prefix_exists() {
    if [[ ! -d "$WINE_PREFIX" ]]; then
        ...
        log_info "B4A may already be uninstalled, or was installed elsewhere."
        return 1
    fi
    return 0
}
...
#-------------------------------------------------------------------------------
echo -e "\n${RED}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${RED}║  B4A Wine Uninstaller for Linux Mint                   ║${NC}"
echo -e "${RED}╚════════════════════════════════════════════════════════╝${NC}\n"

# Check if B4A prefix exists
...
if [[ "$DRY_RUN" != "true" ]]; then
    if [[ "$FORCE" != "true" ]]; then
        if ! confirm "⚠️ This will permanently delete B4A and all associated data"; then
...
#-------------------------------------------------------------------------------
# 1. Stop any running B4A/Wine processes
#-------------------------------------------------------------------------------
...
#-------------------------------------------------------------------------------
# 4. Remove Wine Prefix (Main B4A Installation)
#-------------------------------------------------------------------------------
...
if [[ "$DRY_RUN" != "true" ]]; then
    ...
    find "${HOME}" -maxdepth 3 -name "*B4A*" -o -name "*b4a*" 2>/dev/null | grep -E "\.(exe|zip|tmp|log)$" | while read -r file; do
        rm -f "$file" 2>/dev/null && log_info "Removed residual: $file" || true
    done
    log_success "Residual cleanup completed"
else
    echo "  [DRY RUN] Would clean residual B4A-related temp files"
fi
...
if [[ "$DRY_RUN" == "true" ]]; then
    ...
    echo -e "${GREEN}  ✓ B4A Wine Uninstall Complete!${NC}"
    echo -e "${GREEN}════════════════════════════════════════════════════════${NC}"
    echo -e "\n${GREEN}🗑️  Removed:${NC}"
    ...
    echo -e "\n${YELLOW}📋 Notes:${NC}"
    echo "  • Your B4A source code projects are safe if you kept ~/B4A_Projects"
    echo "  • Wine can be reinstalled anytime: sudo apt install winehq-stable"
    echo "  • To reinstall B4A: run install_b4a_wine.sh again"
    
    echo -e "\n${GREEN}✨ Thank you for using B4A on Linux! Come back anytime! 🚀${NC}"
fi
echo ""
